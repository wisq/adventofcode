defmodule Pulses do
  ...
  defmodule LCMConjunction do
    @enforce_keys [:pulses]
    defstruct(pulses: %{})
  end

  defmodule State do
    @enforce_keys [:layout, :pushes]
    defstruct(
      pushes: 0,
      layout: nil,
      pending: :queue.new(),
  ...
  def push_button(layout, pushes) do
    ...
    |> execute_pending()
    |> then(fn
      %State{solution: nil} = state ->
        state.layout
        |> push_button(pushes + 1)

      %State{solution: n} = state when is_integer(n) ->
        state
  ...
  defp handle_pulse(state, %LCMConjunction{} = module, name, source, :high) do
    pulses =
      module.pulses
      |> Map.update!(
        source,
        fn
          nil ->
            {:measuring, state.pushes}

          {:measuring, old} ->
            case old * 2 == state.pushes do
              true -> {:locked, old}
            end

          {:locked, old} ->
            case rem(state.pushes, old) == 0 do
              true -> {:locked, old}
            end
        end
      )
      |> IO.inspect(label: inspect(name))

    state = state |> update_module(name, %LCMConjunction{module | pulses: pulses})

    case Enum.all?(pulses, fn
           {_, {:locked, _}} -> true
           {_, _} -> false
         end) do
      true -> %State{state | solution: solve_lcm_conjunction(pulses)}
      false -> state
    end
  end
  ...
  defp solve_lcm_conjunction(pulses) do
    pulses
    |> Enum.map(fn {_, {:locked, n}} -> n end)
    |> Enum.reduce(&lcm/2)
  end
  ...
  defp reverse_lookup(layout, name) do
    layout
    |> Enum.filter(fn {_, %{targets: targets}} -> name in targets end)
  end

  defp update_critical_node(layout) do
    [{name, %Conjunction{memory: memory}}] = reverse_lookup(layout, :rx)

    module = %LCMConjunction{
      pulses: memory |> Map.new(fn {key, _} -> {key, nil} end)
    }
